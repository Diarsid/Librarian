...
public final class PatternToWordMatchingV40 extends LoggingInScript implements PatternToWordMatchingCodeV2 {
    ...
    public long evaluate(String pattern, String word) {
        ...
        boolean typoXyYxInCurr = false;
        boolean backwardMatchInPrev = false;
        boolean typoXyYxInPrev = false;
        ...
        wordCharsIterating: for (int i = 0; i < wordLength; i++) {
            ...
            if ( wcInPattern < 0 ) {
                if ( i == 0 ) {
                    ...
                    if ( typoXyYxInPrev ) {
                        char wcX = word.charAt(i - 3);
                        char pcX = pattern.charAt(wcPrevInPattern - 1);
                        if ( wcX != pcX ) {
                            logln("        prev is xy-yx typo, deny typo!");
                            order--;
                            matchInPattern--;
                            matchFull--;
                            matchFull--;
                            ...
                            logln("        prev is xy-yx typo, typo confirmed");
                        }
                    }

                    int wcInPatternWeak = pattern.indexOf(wc, firstWcInPatternIndex);
                    if ( wcInPatternWeak < 0 ) {
                        logln("      WORD:%s not found from word-in-pattern beginning [PATTERN:%s]", wc, firstWcInPatternIndex);
                                ...
                                if ( i != lastInWord ) {
                                    if ( wcInPatternWeak != firstWcInPatternIndex ) {
                                        if ( wcInPatternWeak == wcPrevInPattern - 1 ) {
                                            if ( i == iPrev + 1 ) {
                                                if ( gaps > 0 ) {
                                                    logln("      WORD:%s[PATTERN:%s] backward match [+1] from word-in-pattern beginning [PATTERN:%s] before WORD:%s[PATTERN:%s]",  wc, wcInPatternWeak, firstWcInPatternIndex, wcPrev, wcPrevInPattern);
                                                    backwardMatches++;
                ...
                else if ( firstWcInPatternIndex < 0 && firstFoundWcInPatternIndex < 0 ) {
                    ...
                    if ( distanceFromCPrevToC > 1 ) {
                        ...
                        if ( distanceFromCPrevToC > 2 && distanceFromCPrevToC >= matchInPattern) {
                            ...
                            if ( matchFull < 1 ) {
                                ...
                                if ( gaps > 0 ) {
                                    int from = firstWcInPatternIndex > -1 ? firstWcInPatternIndex : firstFoundWcInPatternIndex;
                                    if ( from > -1 ) {
                                        from++;
                                        char pc;
                ...
                wcPrev = wc;
                if ( firstWcInPatternIndex > -1 ) {
                    wordInPatternLength = wcInPattern - firstWcInPatternIndex + 1;
                }
                else {
                ...
                if ( wcInPattern == lastInPattern ) {
                    if ( rangeLength < 6 && found < matchLength && gaps > 0 ) {
                        wcPrevInPattern = firstWcInPatternIndex > -1 ? firstWcInPatternIndex : firstFoundWcInPatternIndex;
                        continue wordCharsIterating;
                    }
                    else {
        ...
        int matchIndex;
        if ( firstWcInPatternIndex > -1 ) {
            matchIndex = firstWcInPatternIndex;
        }
        else {
            matchIndex = firstFoundWcInPatternIndex;
        }
                ...
                if ( matchType == PATTERN_IS_WORD_FRAGMENT || matchInPatternStrengthBonus == 0 ) {
                    if ( diffInPatternSum > 0 || secondCharMissed == 1 ) {
                        if ( diffInWordSum <= found/2 ) {
                            ...
                            if ( found < 5 || wordFoundRatio < 70 || order < found ) {
