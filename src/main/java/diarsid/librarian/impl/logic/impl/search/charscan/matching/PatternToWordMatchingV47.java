...
public final class PatternToWordMatchingV47 extends PatternToWordMatchingCodeV2 {
...
//                                        }
                ...
                if ( wcInPattern == lastInPattern ) {
                    if ( rangeLength < 6 && found < matchLength && gaps > 0 ) {
                        ...
                        if ( i < 2 && firstFoundWcInPatternIndex > 0 ) {
        ...
        boolean patternStartsWithWord = firstFoundWcInPatternIndex == 0;
...
//                    firstCharMissed++;
        ...
        int diffWordAndPatternSum = diffInWordSum + diffInPatternSum;
        ...
        if ( wordChar_1_Missed  == 0 ) {
            if ( found < 6 && order < found ) {
                if ( diffInPatternSum > 0 && diffWordAndPatternSum > found ) {
        ...
        if ( found == 3 && order == found && backwardMatches == 0 ) {
            if ( matchSpanInPattern < 6 && longestDiffInWord == 1 && patternStartsWithWord && diffInPatternSum < 2 ) {
                if ( wordNotFoundLength < 3 ) {
                    ...
                    rate = rate + 20;
                    logln("   +20 special rule [1.2] - [PATTERN]ace : [WORD]AbCdExx.....>=3");
                }
            }
            if ( matchInPatternStrong == 2 || strongWordEnd ) {
                ...
                if ( patternStartsWithWord ) {
                    if ( diffWordAndPatternSum > 2 ) {
                        logln("     -10 diffInWordSum + diffInPatternSum > 2");
                        rate = rate - 10;
                    }
                }
                else {
                    if ( diffWordAndPatternSum > 1 ) {
                        if ( lastFoundCharIsLastInWord &&
                                diffInWordSum > 0 &&
                                diffInPatternSum == 0 &&
                ...
                if ( patternStartsWithWord ) {
                    ...
                    if ( lastFoundCharIsLastInWord && wordLength < 7 ) {
                        ...
                        else if ( diffWordAndPatternSum > 1 ) {
                            logln("     -10 matchInPatternStrong == 0 && diffInWordSum + diffInPatternSum > 1");
                            rate = rate - 10;
                        }
                    }
                    else if ( diffWordAndPatternSum > 1 ) {
                        logln("     wordLength > 6 word last char is not last found char, diffInWordSum + diffInPatternSum > 1");
                        rate = rate - 10;
                    }
                }
            }
        ...
        if ( found == 4 ) {
            if ( diffInPatternSum > 0 && diffInWordSum > 0 ) {
                ...
                if ( diffWordAndPatternSum > found && (typoMatches > 0 || backwardMatches > 0) ) {
                    rate = rate - 30;
...
//                        rate = rate + 15;
        ...
        if ( found > 4 ) {
            if ( diffInPatternSum > 0 && diffInWordSum > 0 ) {
                ...
                if ( typoMatches > 0 && diffWordAndPatternSum >= found && longestDiffInWord > 2 ) {
                    rate = rate - 30;
                    logln("     -30 typo, wordDiff > 0, patternDiff > 0, longest diff > 2");
                }

                if ( matchType == WORD_IS_PATTERN_FRAGMENT && ( patternLength - wordInPatternLength ) <= 3 ) {
                    if ( diffWordAndPatternSum >= found ) {
                        rate = rate - 10;
...
//                    }
                    ...
                    if ( found > 3 ) {
                        ...
                        else if ( diffWordAndPatternSum <= patternMatchesOnly ) {
                            rate = rate + 20;
                            logln("     +20 [prxu]");
                        }
                        else {
