...
public class EntriesSearchByCharScan extends ThreadBoundTransactionalEntries implements EntriesSearchByPattern {
    ...
    public EntriesSearchByCharScan(Jdbc jdbc, UuidSupplier uuidSupplier) {
        ...
        this.sqlSelectEntriesUuidsByAnyOfLabels = new StringCacheForRepeatedSeparatedPrefixSuffix(
                ...
                "	SELECT \n" +
                "		we.entry_uuid, \n" +
                "		EVAL_MATCHING_V56(?, string) AS word_code \n" +
                "	FROM words w \n" +
                ...
                "	WHERE \n" +
                "       w.user_uuid = ? AND \n" +
                "       EVAL_LENGTH_V11(?, w.string, ?, w.string_sort, 60) > -1 \n" +
                "), \n" +
                "entries_scan AS ( \n" +
                "   SELECT entry_uuid AS uuid, EVAL_CODES_V32(word_code) AS r_code \n" +
    ...
    private List<Entry> searchBy(User user, String pattern) {
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        "), \n" +
                        "entries_scan AS ( \n" +
                        "	SELECT we.entry_uuid, EVAL_CODES_V32(ws.w_code) r_code \n" +
                        "	FROM words_scan ws \n" +
                        "		JOIN words_in_entries we \n" +
                        ...
                        "WHERE es.r_code > 0 ; \n",
                        pattern, pattern, transform(pattern), user.uuid())
                .collect(toList());
    ...
    private List<Entry> searchByLabel(String pattern, Entry.Label label) {
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        "	SELECT \n" +
                        "		we.entry_uuid, \n" +
                        "		EVAL_MATCHING_V56(?, w.string) AS word_code \n" +
                        ...
                        "FROM entries_scan \n" +
                        "WHERE r_code > -1 ",
                        pattern, label.uuid(), label.userUuid(), pattern, transform(pattern))
                .collect(toList());
    ...
    private List<Entry> searchByAnyOfLabels(String pattern, List<Entry.Label> labels) {
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        pattern, uuidsOf(labels), labels.get(0).userUuid(), pattern, transform(pattern))
                .collect(toList());
    ...
    private List<Entry> searchByAllOfLabels(String pattern, List<Entry.Label> labels) {
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        pattern, uuidsOf(labels), labels.size(), labels.get(0).userUuid(), pattern, transform(pattern))
                .collect(toList());
    ...
    private List<Entry> searchByBefore(User user, String pattern, LocalDateTime time) {
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        "   es.r_code > 0 AND \n" +
                        "   e.time < ?; \n",
                        pattern, pattern, transform(pattern), user.uuid(), time)
                .collect(toList());
    ...
    private List<Entry> searchByLabelBefore(String pattern, Entry.Label label, LocalDateTime time) {
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        "FROM entries_scan \n" +
                        "WHERE r_code > -1 ",
                        pattern, label.uuid(), label.userUuid(), pattern, transform(pattern), time)
                .collect(toList());
    ...
    private List<Entry> searchByAnyOfLabelsBefore(String pattern, List<Entry.Label> labels, LocalDateTime time) {
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        pattern, time, uuidsOf(labels), labels.get(0).userUuid(), pattern, transform(pattern))
                .collect(toList());
    ...
    private List<Entry> searchByAllOfLabelsBefore(String pattern, List<Entry.Label> labels, LocalDateTime time) {
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        pattern, time, uuidsOf(labels), labels.size(), labels.get(0).userUuid(), pattern, transform(pattern))
                .collect(toList());
    ...
    private List<Entry> searchByNotLabel(String pattern, Entry.Label label) {
        ...
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        "FROM entries_scan \n" +
                        "WHERE r_code > -1 ",
                        pattern, label.uuid(), userUuid, userUuid, pattern, transform(pattern))
                .collect(toList());
    ...
    private List<Entry> searchByNoneOfLabels(String pattern, List<Entry.Label> labels) {
        ...
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        pattern, uuidsOf(labels), userUuid, userUuid, pattern, transform(pattern))
                .collect(toList());
    ...
    private List<Entry> searchByNotLabelBefore(String pattern, Entry.Label label, LocalDateTime time) {
        ...
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        "FROM entries_scan \n" +
                        "WHERE r_code > -1 ",
                        pattern, time, label.uuid(), userUuid, userUuid, pattern, transform(pattern))
                .collect(toList());
    ...
    private List<Entry> searchByNoneOfLabelsBefore(String pattern, List<Entry.Label> labels, LocalDateTime time) {
        ...
        List<UuidAndAggregationCode> entryUuidsCodes = super.currentTransaction()
                .doQueryAndStream(
                        ...
                        pattern, time, uuidsOf(labels), userUuid, userUuid, pattern, transform(pattern))
                .collect(toList());
